...
!!! warning "务必知晓什么是局域网"
    ...
    如果你的目标是在局域网外访问 SillyTavern，请参考[本文档中的云端部署部分](cloud_deploy/vps.md)。

## 确定需要共享酒馆的设备在局域网中的 IP 地址

!!! info "在大多数家用环境里，局域网的 IP 地址可能会偶尔发生变化"

    你可能需要时常更新客户端的 IP 地址。
    如果你希望在局域网中使用一个静态 IP 地址，可以在操作系统的网络设置，或路由器的 DHCP 设置中为你的设备分配一个静态 IP 地址。
